...
const LawyerView = () => {
  ...
  const { data, loading, execute } = useAsync((customerId) => axios.get(`https://tqneen-testing-be1-dot-tqneen-406411.ew.r.appspot.com/api/customer/lawyers/${customerId}`, { headers: { Authorization: `Bearer ${token}` } }), { immediate: false })

  const { data: updatedCustomer, loading: isupdatedCustomerLoading, execute: updateLawyer } = useAsync(({ customerId, lawyerData }) => axios.put(`https://tqneen-testing-be1-dot-tqneen-406411.ew.r.appspot.com/api/admin/users/${customerId}`, lawyerData, { headers: { Authorization: `Bearer ${token}` } }), { immediate: false })
