...
const LawyerView = () => {
  ...
  const { data, loading, execute } = useAsync((lawyerId) => axios.get(`https://tqneen-testing-be1-dot-tqneen-406411.ew.r.appspot.com/api/customer/lawyers/${lawyerId}`, { headers: { Authorization: `Bearer ${token}` } }), { immediate: false })
  ...
  const { data: updatedlawyer, loading: isUpdatedLawyerLoading, execute: updateLawyer } = useAsync(({ lawyerId, lawyerData }) => {
    ...
    return axios.put(
      `https://tqneen-testing-be1-dot-tqneen-406411.ew.r.appspot.com/api/admin/users/${lawyerId}`,
      {
